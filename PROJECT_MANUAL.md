...
4. **The deprecated `?token=` WebSocket fallback** is still accepted. Production logs show real clients still using it (cached bundles), so it is currently load-bearing — remove once traffic has moved.
5. **Rate-limit counters are in-process** — a single uvicorn worker, reset on every restart and redeploy. Needs a shared store before scaling out, and a determined attacker can reset their budget by waiting for a deploy.
...
7. **AI spend has no per-user budget.** `/manager/command` is rate limited (30/min) but nothing caps how much a single account can spend on model calls.
