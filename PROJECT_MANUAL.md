...
- **Regression tests:** `backend/tests/test_security_phase0.py` (12) and `test_security_phase1.py` (13) each *attempt the real exploit* and assert it fails, with positive controls so nothing passes vacuously — including an end-to-end RAG test that indexes private-channel content and proves a non-member's AI search can't retrieve it. Suite: **39 passed**.
- **Still open (later phases):** login's SQL `LIKE` wildcards aren't escaped; rate limiting trusts `X-Forwarded-For`; no request-body size cap; WS tokens ride in the query string (logged by proxies); no password-strength rule or refresh rotation; untrusted-content framing not yet applied to Slack/RAG/MCP/file content; MCP lacks an SSRF guard; no security headers and `/docs` is public; `token_crypto` derives its key without HKDF and has a hardcoded fallback. `SETUP_SECRET` is still on its default (one-time first-manager only); full multi-tenant `company_id` scoping inside the orchestrator tools is deferred (the pilot is single-tenant-per-instance).
