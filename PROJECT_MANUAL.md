...
- ### Post-audit hardening (2026-08-02)

Three items from the "what's still open" list, done after the six phases.

- **AI spend cap** (`api/spend.py`, `ai_spend` table). Rate limiting bounds how MANY requests an account makes, not what they cost — one long agentic turn with a dozen tool rounds is worth hundreds of trivial ones, so 30/minute was never a spending limit. Cost was already measured accurately but only emitted to the live circuit board, so it vanished on restart: nothing could answer "what did we spend yesterday", let alone stop a runaway. Every call is now recorded and two ceilings are checked before any paid call — `NEXUS_DAILY_USD_PER_USER` (default $5) and `NEXUS_DAILY_USD_PER_COMPANY` (default $50, the backstop for a runaway loop in background work that no per-user cap would see). `GET /analytics/ai-spend` (manager-only) shows spend by person and the active limits. Three decisions: attribution is by PERSON (`Manager_1` is a role label, not an employee id — left unresolved it filed the manager's usage under "background jobs" and exempted the heaviest account from the cap); genuinely person-less work counts toward the company total but is never billed to a user nor blocked by a personal cap, so one employee's exhausted budget can't stop the nightly digest; and the check **fails open**, because taking the assistant offline over a bookkeeping error is worse than one turn over budget. `estimate_cost` returns $0 for an unpriced model, which would silently disable the budget — `record()` now warns loudly and a test asserts every model the code actually calls is in `PRICING`.
- **Brute-force throttling that an IP change doesn't defeat** (`api/login_guard.py`, `login_attempts` table). Login throttling was keyed only on the caller's address: an attacker rotating addresses never reached the limit, and the counters died with the worker. Adding Redis would have fixed the second problem and not the first — it is a design gap, not a storage gap. Failures are now recorded and throttled per **account** as well as per address, survive a restart, and count attempts against names that don't exist so a spray across guessed names is visible. A successful sign-in clears that account's failures. Old rows are purged hourly. **Verified live: 14 guesses against one account from 14 different addresses gave 10 rejections then a block, with the per-IP limiter never firing, while an unrelated account from a fresh address still signed in.** Stated trade-off: someone who knows a colleague's name can push that account into its cool-off — the right way round, since an account lockable for fifteen minutes is a nuisance and an account guessable at unlimited speed is a breach.
- **Retiring the `?token=` WebSocket fallback is now a config flip** (`NEXUS_ALLOW_WS_QUERY_TOKEN`, default on). Production logs showed 6 of 10 handshakes still arriving that way from two employees — browser tabs open since before the subprotocol shipped — so removing it today would break live sessions. Switch it off when the deprecation warnings stop appearing.
- **`SECURITY_PENTEST_BRIEF.md`** (repo root) scopes the external test: what the system is, the three defences worth attacking, a ranked starting list, what has already been done so budget isn't spent re-finding it, and an honest known-open table. Commissioning that test remains the highest-value security step available — every fix in this file was written and verified by the same agent.

**Still open, after all six phases (2026-08-02).** Everything the phases covered is closed and re-verified; these are the known remaining gaps, stated plainly rather than left implied:
1. **No independent review.** Every fix here was written and verified by the same agent. That is the largest open risk in this list — self-assessment is not an audit, and an external pentest would be the single highest-value next step.
2. **Prompt-injection defence is mitigation, not prevention.** Untrusted-content framing reduces how often the model is fooled; the thing that actually STOPS an outward action is the non-LLM approval gate. Treat any claim that injection is "solved" as false.
3. **MCP DNS rebinding** — the URL guard validates at submission, not at fetch; closing it needs pinning the resolved IP into the connection.
4. **The deprecated `?token=` WebSocket fallback** is still accepted, now behind `NEXUS_ALLOW_WS_QUERY_TOKEN` — live clients still use it, so switch it off once the deprecation warnings stop.
5. **General rate-limit counters are still in-process** — reset on restart, single worker. Login throttling is now database-backed and account-aware, so the brute-force case is covered; the rest needs a shared store before scaling out.
6. **Single-tenant per instance.** Orchestrator tools hardcode `company_id=1`; full multi-tenant scoping inside the tool layer is deferred by design.
Verified NOT open, contrary to earlier notes in this file: `SETUP_SECRET` is set to a strong value in both environments (production `/auth/setup` answers 403 on the old default, i.e. configured and non-default), `token_crypto` uses HKDF with a rotation runbook (§20a), and route-coverage, tool-authorization and dependency scanning all run in CI.
